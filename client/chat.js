...
const preferredLanguage = prompt("Enter your preferred language:")
...
socket.emit("new-connection", { username,preferredLanguage });
...
messageForm.addEventListener("submit", (e) => {
  ...
  const destUser = prompt("Who do you want to send this message?")

  // check if there is a message in the input
  if (messageInput.value !== "") {
    let newMessage = messageInput.value;
    //sends message and our id to socket server
    socket.emit("new-message", { user: socket.id, destusername: destUser, message: newMessage });
